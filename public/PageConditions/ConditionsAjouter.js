...
async function boutonajouter() {
    ...
    if (option == 3 || option == 4) {
        const ajout = await fetch(`http://localhost:3000/ajoutercondition/${3}/${8}/${text}`, settings)
        if (ajout.ok) {
        ...
        const ajout = await fetch(`http://localhost:3000/ajouterconditionadresse/${8}/${`${text} `}/${3}/${conditions1}`, settings)
        if (ajout.ok) {
            alert("Condition Ajouter")
        } else {
            console.error('ERROR');
        ...
        let conditions1 = document.getElementById("inputconditions1").value
        const ajout = await fetch(`http://localhost:3000/ajouterconditionvictime/${3}/${8}/${`${text} `}/${conditions1}`, settings)
        if (ajout.ok) {
            alert("Condition Ajouter")
        } else {
            console.error('ERROR');
        ...
        let conditions1 = document.getElementById("inputconditions1").value
        const ajout = await fetch(`http://localhost:3000/ajouterconditionfrequentation/${3}/${8}/${`${text} `}/${conditions1}`, settings)
        if (ajout.ok) {
            alert("Condition Ajouter")
        } else {
            console.error('ERROR');
        ...
        const ajout = await fetch(`http://localhost:3000/ajouterconditionheure/${3}/${8}/${text}/${conditions2}/${conditions3}`, settings)
        if (ajout.ok) {
            alert("Condition Ajouter")
        } else {
            console.error('ERROR');
